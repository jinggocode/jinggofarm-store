...
<section class="content">
  ...
  <div class="box box-success">
    ...
    <div class="box-body">
      ...
      <form class="form-horizontal" action="{{site_url('admin/kurir/ongkir/update')}}" method="post">
        {{$csrf}} 
          ...
          <div class="col-sm-6">
            <select name="id_kurir" id="id_kurir" class="form-control" style="width: 100%;">
              ...
              @foreach ($kurir as $value)
                <option {{($data->id_kurir == $value->id)?'selected':''}} value="{{$value->id}}">{{$value->nama_kurir}}</option>                   
              @endforeach
            </select>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <input type="text" name="biaya" class="form-control" value="{{$data->biaya}}" id="biaya">
          </div>
        </div>
        <div class="form-group">
...
$(document).ready(function(){
  ...
  var kec = <?php echo $data->id_kecamatan; ?>;

  $.ajax({
      type : 'GET',
      url : '<?php echo base_url('admin/kurir/ongkir/show/getKabupaten'); ?>',
  ...
  $.ajax({
      type : 'GET',
      url : '<?php echo base_url('admin/kurir/ongkir/show/getKecamatan'); ?>',
