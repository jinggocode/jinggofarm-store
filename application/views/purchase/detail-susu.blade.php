                  ...
                  <div class="col-6">
                    <div class="d-flex align-items-center"><img src="{{base_url('uploads/product/'.$value->product->foto)}}" class="img-fluid">
                      <div class="title">
                        ...
                        @if ($pembelian->status == '3')
                          {{($pembelian->status === '3' AND $value->status_testimoni === '0')?'<a id="testimoni" data-id="'.$value->id.'" data-id_produk="'.$value->product->id.'" data-name="'.$value->product->nama.'" class="btn btn-light text-white btn-sm" data-toggle="modal" data-target=".bd-testimoniModal"><i class="fa fa-pencil-square"></i> Beri Testimoni</a>':'<span class="badge badge-light"><i class="fa fa-check"></i> Sudah diulas</span>'}} 
                        @endif
                      </div>
                    </div>
                  </div>
        ...
        <div class="row addresses">
          <div class="col-sm-12">
            ...
            <div class="block-body">
              <p>{{$kabupaten->name}}, {{$provinsi->name}} <br>{{$pembelian->detail_alamat}} </p>
            </div>
          </div>
        </div>
        <!-- /.addresses                           -->
